...
def addFriend(request, username):
    """
    Friendsテーブルに引数で受け取ったユーザ名を友達として登録する。
    """
    login_user = request.user.username
    friend = CustomUser.objects.get(username=username)
    curr_user = CustomUser.objects.get(username=login_user)
    ls = curr_user.user_friends.all()

    #既に登録済みの場合flag=1にセット
    flag = 0
    for username in ls:
        if username.friend.pk == friend.pk:
            flag = 1
            break
    ...
    if flag == 0:
        ...
        #お互いにフレンド登録を行う。
        curr_user.user_friends.create(friend=friend)  #ログオンユーザ視点でフレンドを登録
        friend.user_friends.create(friend=curr_user)   #フレンド視点でログオンユーザをフレンドに登録
...
def get_message(request, username):
    ...
    """
    friend = CustomUser.objects.get(username=username)
    curr_user = CustomUser.objects.get(username=request.user.username)
    messages = Messages.objects.filter(sender_name=curr_user.id, receiver_name=friend.id) | \
               Messages.objects.filter(sender_name=friend.id, receiver_name=curr_user.id)
